...
def generate_stk_connect_commands(semi_maj_axis, eccentricity, orbit_data, number_of_sps, mean_anomalies, time_step, study_name, file_path):
    ...
    with open('CC_{}_OrbitStudy.txt'.format(study_name), 'w') as fh:
        ...
        for i in range(len(semi_maj_axis)):
            # determine max constellation size for orbit
            constellation_size = number_of_sps[i]
            # loop through constellation sizes
            for j in range(2, constellation_size + 1):
                mean_anom = mean_anomalies['{}sps'.format(j)]
                ...
                for k in mean_anom:
                    ...
                    fh.write('SetState */Satellite/SPS1 Classical J4Perturbation "17 May 2018 10:00:00.000" "17 May 2020 '
                             ...
                             '90 90 0 {}\n'.format(time_step, semi_maj_axis[i]*1000, eccentricity[i], k))
                    ...
                    fh.write('ReportCreate */Satellite/SPS1 Type Save Style "Access_Range_Stats" File "{}\DVP_{}_{}perigee{}apogee_{}sps_{}meananom__range.txt" AccessObject '
                             '*/Target/Target1\n'.format(file_path, study_name, orbit_data[i + 1][0], orbit_data[i + 1][1], j, k))
                    # Generates new report of lighting times
                    fh.write('ReportCreate */Satellite/SPS1 Type Export Style "Lighting_Times" File "{}\DVP_{}_{}perigee{}apogee_{}sps_{}meananom__'
                             'lighting.csv"\n'.format(file_path, study_name, orbit_data[i + 1][0], orbit_data[i + 1][1], j, k))
    time_end = time.time()
...
def main():
    ...
    scenario_path = '{}/STK-Scenarios/PolarOrbit/__________.sc'.format(main_directory)
    ...
    # number_of_sps, mean_anomalies = determine_constellation_size(ecc)
