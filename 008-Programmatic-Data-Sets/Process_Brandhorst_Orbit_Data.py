...
def main():
    ...
    semi_maj_axis, eccentricity, orbit_data = get_orbital_elements()

    # Import report data generated via STK Connect Commands
    total_active_time = []
    max_active_duration = []
    total_blackout_time = []
    max_blackout_duration = []

    # For accessing the file in a folder contained in the current folder
    fileDir = os.path.dirname(os.path.realpath('__file__'))
    # Import target illumination events
    target_lighting_raw = 'Brandhorst_250kmRes/Target_Lighting.csv'
    ...
    #     sps_lighting_raw = os.path.join(fileDir, 'Brandhorst_250kmRes/{}perigee{}apogee_lighting.csv'.format(orbit_data[i][0], orbit_data[i][1]))
    #     sps_lighting = parse_csv_to_array(sps_lighting_raw, start)
    #     sps_access_raw = os.path.join(fileDir, 'Brandhorst_250kmRes/{}perigee{}apogee_access.csv'.format(orbit_data[i][0], orbit_data[i][1]))
    ...
    #     max_blackout_duration.append(max(target_blackout[2]))

        # Write data to a file so that the file can be read as opposed to importing and processing
        # # data every time. This can help speed up debugging/analysis
        # file1 = open('total_active_times_250kmRes.txt', 'a')
        # file1.write("{}\n".format(np.sum(sps_active[2])))
        # file2 = open('max_active_times_250kmRes.txt', 'a')
    ...
    total_active_time = []
    fh = open('total_active_times_1000kmRes.txt')
    ...
    # for k in range(50):
    #     print('Perigee: {} km, Apogee: {} km'.format(round(unique_perigees[best_orbit_idx[0]], 2),
    #                                                  round(unique_apogees[best_orbit_idx[1] - k], 2)))
    #     print('Active time: {} hrs'.format(round(active_times[best_orbit_idx[0] - k][best_orbit_idx[1] - k]), 2))
    #     print('Perigee: {} km, Apogee: {} km'.format(round(unique_perigees[best_orbit_idx[0] - k], 2),
    #                                                  round(unique_apogees[best_orbit_idx[1]], 2)))
    #     print('Active time: {} hrs'.format(round(active_times[best_orbit_idx[0] - k][best_orbit_idx[1] - k]), 2))
    ...
    plt.title('Total Active Time, Brandhorst Configuration')
