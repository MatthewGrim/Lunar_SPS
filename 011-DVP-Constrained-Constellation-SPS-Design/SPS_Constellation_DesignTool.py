...
from SPS_Constellation_DesignOptimizer import optimize_link_efficiency
...
def generate_design_space(study_name, rover_selection, transmitter_selection, constraints, active_constraints):
    ...
    ####################################################################################################################
    # Based on desired total active time, determine how many SPS are required in the constellation
    # Set maximum SPS constellation size for which data is available
    max_num_sps = 2
    num_sps = determine_number_of_sps_for_active_time(stk_data_path, study_name, study, constraints, active_constraints, max_num_sps)
    ####################################################################################################################
    ...
    if num_sps > 1:
        print('Total system mass --> {} kg'.format(round(2.0 * (generator_mass + transmitter['mass']), 2)))
